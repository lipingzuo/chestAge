...
def apply(image, model=None, force_cpu=False, batch_size=20, volume_postprocessing=True, noHU=False):
    ...
    with torch.no_grad():
        for it, X in  enumerate(dataloader_val):
            X = X.float().to(device)
            prediction = model(X)
            pls = torch.max(prediction, 1)[1].detach().cpu().numpy().astype(np.uint8)
            timage_res = np.vstack((timage_res, pls))
            if it % (int(len(dataloader_val) / 5)) == 0:
                logger.info('[LungClip] infer {}/{}'.format(it + 1, len(dataloader_val)))
    ...
    if volume_postprocessing:
        ...
        logger.info('[LungClip] post process cost {:.2f}s'.format(time.time()-time_s))
    else:
        outmask = timage_res
